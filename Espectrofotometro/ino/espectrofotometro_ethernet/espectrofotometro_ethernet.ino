...
/////////////////ETHERNET//////////////////////////////////////////////
 ...
 // byte subnet[] = { 255, 255, 255, 0 }; // setup manual apenas
  ...
  EthernetServer server(23); //porta '80'
...
Stepper myStepper(stepsPerRevolution, 4, 5, 6, 7); // initialize the stepper library on pins 8 through 11.
  ...
  int LED=50; // acende o led quando a varredura direita ocorre
...
//motores contagem distância
  ...
  float cnt;
  int intervaloIda = 8; //define a VARIÁVEL "intervaloIda" igual a 10, pode ser alterado pelo usuario, representa o intervalo de passos do motor para cada captura de dados
  int velocidadeIda = 30; //define a VARIÁVEL "velocidadeIda" igual a 100. Esta variável é a velocidade da varredura do sensor na Ida, pode ser alterada pelo usuario
...
//irradiancia////
  ...
  int ReleLiga=51; //rele aciona se estiver alto no pino 7, ligando a lâmpada (ou seja, rele acionado, liga a lâmpa quando varredura direita ocorre)
...
// Pinagem para a medição do US//
  int echoPinUS = 22; //eco - recebe
  int trigPinUS = 23; // trigger - envia
...
void setup() {
  ...
  myStepper.setSpeed(60); //ajuste de velocidade dos passos
...
void MoverCabeca() { 
  ...
  if(movimentacaoCabeca == 1) {
    ...
    server.println("Ajuste de Posicao Frente");
  ...
  if(movimentacaoCabeca == 2) {
    ...
    client.println("Ajuste de Posicao Tras");
  ...
  if(movimentacaoCabeca == 0){
    ...
    client.print("Pare aqui: encontramos a fenda");
    Serial.println("Pare aqui: encontramos a fenda");
...
///////IMPRESSÃO DOS DADOS//////////////////
    server.print(irradiance); // print the frequency (pulses/second)
    Serial.print(irradiance); // print the frequency (pulses/second)
    server.print(";"); // Imprime em caracteres ASCII " "
    Serial.print(";"); // Imprime em caracteres ASCII " "
    server.println(compdeonda, 3); //Imprime em caracteres ASCII a variável "compdeonda"
    Serial.println(compdeonda, 3); //Imprime em caracteres ASCII a variável "compdeonda"    
    delay (200); // wait 4 seconds until the next measurement
}
